...
// OnlineFCMReq is the payload forwarded to alarm-api to associate an FCM token with an API token.
...
// This will be sent to alarm-api to store that data in Redis to be able to send Push Notifications
...
func (ft *FCMToken) initFCMTokenViaHTTP(obj *OnlineFCMReq) error {
	...
	// do the real call to the remote alarm-api service
