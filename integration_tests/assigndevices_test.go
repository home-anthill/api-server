	...
	Context("calling assignDevice api PUT", func() {
		...
		When("device is still not assigned to any home+room", func() {
			...
			It("should not assign device, because homeId is not a home in db", func() {
				...
				err := testuutils.AssignDeviceToProfile(ctx, collProfiles, profileRes.ID, deviceController.ID)
				Expect(err).ShouldNot(HaveOccurred())
				err = testuutils.AssignHomeToProfile(ctx, collProfiles, profileRes.ID, unknownHomeID)
				Expect(err).ShouldNot(HaveOccurred())
		...
		When("device is already assigned to another room of the same home", func() {
			It("should assign device to home+room and clean previous assignment successfully", func() {
				...
				err = testuutils.AssignDeviceToHomeAndRoom(ctx, collHomes, home.ID, home.Rooms[1].ID, deviceController.ID)
				Expect(err).ShouldNot(HaveOccurred())
				homeFromDb, err := testuutils.FindOneById[models.Home](ctx, collHomes, home.ID)
				Expect(err).ShouldNot(HaveOccurred())
		...
		When("device is already assigned to another home of the same profile", func() {
			It("should assign device to home+room and clean previous assignment successfully", func() {
				...
				err = testuutils.AssignDeviceToHomeAndRoom(ctx, collHomes, home2.ID, home2.Rooms[0].ID, deviceController.ID)
				Expect(err).ShouldNot(HaveOccurred())
				err = testuutils.AssignDeviceToHomeAndRoom(ctx, collHomes, home2.ID, home2.Rooms[0].ID, deviceSensor.ID)
				Expect(err).ShouldNot(HaveOccurred())
				homeFromDb, err := testuutils.FindOneById[models.Home](ctx, collHomes, home.ID)
				Expect(err).ShouldNot(HaveOccurred())
