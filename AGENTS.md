...
- **Device deletion cleanup**: `DELETE /api/devices/:id` must clean downstream state before removing api-server's own device/profile/home references: sensor features in `register`, controller features in `api-devices`, and online state in `online`/Redis. There is no true distributed transaction across MongoDB, HTTP, gRPC, and Redis. If downstream cleanup succeeds but the final api-server MongoDB transaction fails, the API returns an error and the client can retry; downstream deletes are idempotent, so retrying is safe.
