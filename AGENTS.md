...
- **Internal HTTP calls**: Use the shared helpers in `utils/http.go` for calls to sensor/alarm services. They apply a bounded timeout, return non-2xx responses as errors, and centralize response-body handling.
- **Internal service URLs**: Device UUIDs, feature UUIDs, and feature names must be escaped with `url.PathEscape` before being appended as path segments for sensor/alarm service URLs.
- **Device deletion cleanup**: `DELETE /api/devices/:id` must clean downstream state before removing api-server's own device/profile/home references: sensor features in `register`, controller features in `api-devices`, and online state in `alarm`/Redis. There is no true distributed transaction across MongoDB, HTTP, gRPC, and Redis. If downstream cleanup succeeds but the final api-server MongoDB transaction fails, the API returns an error and the client can retry; downstream deletes are idempotent, so retrying is safe.
...
- **Notification device enrichment authorization**: `GET /api/notifications` proxies notification history from the `alarm` service, then replaces each notification's `devices` entries with full device documents from api-server's MongoDB `devices` collection. The lookup must filter by both the UUIDs returned by `alarm` and the authenticated profile's owned device IDs (`profile.Devices`) so a downstream response cannot cause api-server to expose devices outside the logged-in profile.
...
- `HTTP_ALARM_APITOKEN_API` - Internal alarm-service endpoint used when api-server asks alarm to update API-token-backed state
