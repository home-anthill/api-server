...
- **Session/Auth**: The session stores primitive `profileID` and `githubID` values for web-authenticated browser flows. JWT-protected handlers read authenticated identity from validated claims via `utils.GetProfileFromContext()` or `utils.GetLoggedProfileFromContext()` and re-fetch profile data from DB when fresh profile state is required. Mobile app login, protected API calls, and logout do not establish, require, clear, or renew browser session identity.
- **Logging**: Zap `SugaredLogger` throughout; file logging disabled in test mode
- **Indentation**: Tabs for Go files (see `.editorconfig`)
- **JWT security**: Access tokens are signed with `JWT_PASSWORD`, fixed to HS512, and carry `iss`, `aud`, `sub`, `iat`, `nbf`, `exp`, `profileId`, GitHub ID, `clientType`, and `tokenType=access`. The middleware validates issuer/audience, signing method, bearer prefix, and token type. Validated claims are stored in Gin context under key `"jwt_claims"`. Web tokens additionally require session/JWT identity match; mobile tokens are bearer-only and do not depend on browser session state.
...
- **Internal HTTP calls**: Use the shared helpers in `utils/http.go` for calls to register/alarm-api services. They apply a bounded timeout, return non-2xx responses as errors, and centralize response-body handling.
- **Internal service URLs**: Device UUIDs, feature UUIDs, and feature names must be escaped with `url.PathEscape` before being appended as path segments for register/alarm-api service URLs.
- **Device deletion cleanup**: `DELETE /api/devices/:id` must clean downstream state before removing api-server's own device/profile/home references: sensor features in `register`, controller features in `api-devices`, and online state in `alarm-api`/Redis. There is no true distributed transaction across MongoDB, HTTP, gRPC, and Redis. If downstream cleanup succeeds but the final api-server MongoDB transaction fails, the API returns an error and the client can retry; downstream deletes are idempotent, so retrying is safe.
...
- **Notification device enrichment authorization**: `GET /api/notifications` proxies notification history from the `alarm-api` service, then replaces each notification's `devices` entries with full device documents from api-server's MongoDB `devices` collection. The lookup must filter by both the UUIDs returned by `alarm-api` and the authenticated profile's owned device IDs (`profile.Devices`) so a downstream response cannot cause api-server to expose devices outside the logged-in profile.
...
- `HTTP_SENSOR_*` / `HTTP_ALARM_*` - External service endpoints
- `HTTP_ALARM_APITOKEN_API` - Internal alarm-api endpoint used when api-server asks alarm-api to update API-token-backed state
