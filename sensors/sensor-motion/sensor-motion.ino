...
// include libraries:
// NO NEED TO INSTALL EXTERNAL LIBRARIES FOR THIS SENSOR
...
// ------------------------------------------------------
// ----------------------- MOTION -----------------------
#define MOTION_PIN 4 // Digital pin connected to the motion sensor
...
void notifyValue(char* type, bool value) {
...
uint registerServer() {
  ...
  String features = "[";
  features += "{\"type\": \"sensor\",\"name\": \"motion\",\"enable\": true,\"priority\": 1,\"unit\": \"-\"}";
  features += "]";
...
void readSensorValue() {
  ...
  Serial.print("readSensorValue - digital value read = ");
  ...
  if (currentValue != newValue) {
    ...
    bool valueToSend = LOW;
    // convert the int value into a boolean
    if (currentValue == 0) {
      valueToSend = LOW;
    } else {
      valueToSend = HIGH;
    }
    notifyValue("motion", valueToSend);
  }
}
...
void initSensor() {
  ...
  Serial.println("initSensor - pin configured as input to read values");
}
